...
- `scripts`: Contains a script to automatically set which type of control card we want to be active.

### User Task:
- `seeded`: Inspired by [Meta's AI-Enabled Coding interviews](https://www.hellointerview.com/blog/meta-ai-enabled-coding), where interviewees are given a multi-file coding task to complete with an LLM, we provide a Python security authentication task (with unit tests) that has seeded traps the user must debug and resolve with the help of Cursor's LLM agent! Each user was either assigned the condition of no control card (baseline), light control card, or a strong control card (structured review card). Note that no user was assigned more than 1 condition since that could unfairly affect how they perform the second time they are given the task.
